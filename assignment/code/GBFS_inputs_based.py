...
def search_BFS(kitchen_items=[], goal_node=None):
    ...
    while len(items_to_search) > 0:
        ...
        if not check_if_exist_in_kitchen(kitchen_items, current_item):
            ...
            for i, item in enumerate(candidate_units):
                temp_count = candidate_units.count(current_item)
                
                if temp_count < num and temp < i:
                    num = temp_count
                    temp = i
                    index = i


            selected_candidate_idx = candidate_units[index]
            temp = selected_candidate_idx
        ...
        for object in foon_object_nodes:
            if object.check_object_equal(node_object):
                ...
                save_paths_to_file(output_task_tree,
                                   'output_GBFS_input_based_{}.txt'.format(node["label"]))
                break
